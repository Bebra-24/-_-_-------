    ...
    public partial class EmployeeTerminalWindow : Window
    {
        ...
        private void LoadRequests()
        {
            try
            {
                using (var conn = new NpgsqlConnection(connString))
                {
                    ...
                    // Безопасное получение значений из ComboBox
                    string type = null;
        ...
        private void RequestsGrid_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (RequestsGrid != null && RequestsGrid.SelectedItem != null)
            {
                ...
                string requestType = row["тип_заявки"]?.ToString() ?? "";

                var detailWindow = new RequestDetailWindow(requestId, requestType, _employeeId);
